...
export function EnterpriseBeta(){
 ...
 {section==='hotels'&&<section className="rounded-2xl bg-white p-5 lg:col-span-2"><h2 className="mb-4 flex gap-2 font-black"><Hotel className="text-slate-900"/>Hospedajes</h2>{data?.hotels.map(h=><div key={h.link_id} className="mb-2 flex items-center justify-between rounded-xl border p-3"><b className="text-sm">{h.name}</b><div className="flex items-center gap-3">{h.status==='accepted'?<button type="button" onClick={()=>void openHotelDetail(h)} className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200"><Info className="h-3.5 w-3.5"/>Ver detalles</button>:<span className="text-xs font-bold uppercase text-amber-600">{h.status==='pending'?'Pendiente':h.status==='revoked'?'Desincronizado':h.status==='rejected'?'Rechazado':h.status}</span>}</div></div>)}{data?.available_hotels.map(h=><button key={h.id} onClick={()=>void link(h.id)} className="mb-2 flex w-full justify-between rounded-xl border p-3 text-sm font-bold">{h.name}<Plus className="h-4 w-4 text-slate-900"/></button>)}</section>}
