...
type Data={workers:Worker[];hotels:LinkedHotel[];assignments:Assignment[]};
...
const iosDetailGroup='mx-4 grid divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white sm:grid-cols-3 sm:divide-y-0 sm:divide-x';
const iosDetailRow='flex flex-col gap-1.5 px-4 py-3.5';
...
export function EnterpriseBeta(){
 ...
 const [hotelSearchResults,setHotelSearchResults]=useState<{id:string;name:string}[]|null>(null);
 const [hotelSearchBusy,setHotelSearchBusy]=useState(false);
 const [hotelDetail,setHotelDetail]=useState<{tenant_id:string;name:string;contact_phone:string|null;rooms_total:number;rooms_available:number;workers_here:number;workers_here_list:{guest_name:string;guest_dni:string;room_number:string|null;check_in_date:string;check_out_date:string|null}[];razon_social:string|null;ruc:string|null;direccion:string|null;cuenta_bancaria:string|null;cci:string|null;n_detraccion:string|null;fiscal_email:string|null}|null>(null);
 ...
 const [section,setSection]=useState<'home'|'workers'|'hotels'|'tracking'>('home');
 ...
 const link=async(id:string)=>{if(!session)return;setBusy(true);const{error}=await getClient().rpc('enterprise_request_hotel_link',{p_token:session.token,p_tenant:id});setBusy(false);if(error)return setMessage(error.message);setHotelSearchResults(null);setHotelSearch('');await load()};
 const searchHotel=async(e:FormEvent)=>{e.preventDefault();if(!session)return;if(hotelSearch.trim().length<3)return setMessage('Escribe al menos 3 caracteres para buscar.');setHotelSearchBusy(true);const{data:res,error}=await getClient().rpc('enterprise_search_hotel',{p_token:session.token,p_query:hotelSearch.trim()});setHotelSearchBusy(false);if(error)return setMessage(error.message);setHotelSearchResults(res as{id:string;name:string}[])};
 const unlinkHotel=async()=>{if(!session||!hotelDetail)return;if(!confirm(`¿Desincronizar "${hotelDetail.name}"? Dejarás de poder asignarle trabajadores y ver sus datos.`))return;setBusy(true);const{error}=await getClient().rpc('enterprise_unlink_hotel',{p_token:session.token,p_tenant:hotelDetail.tenant_id});setBusy(false);if(error)return setMessage(error.message);setHotelDetail(null);await load()};
 ...
 const exportHotelWorkers=()=>{if(!hotelDetail)return;const rows=hotelDetail.workers_here_list.map(w=>({DNI:w.guest_dni,Nombre:w.guest_name,Habitación:w.room_number||'—','Fecha de entrada':fmtDate(w.check_in_date),'Fecha de salida':w.check_out_date?fmtDate(w.check_out_date):'Sin definir'}));const ws=XLSX.utils.json_to_sheet(rows);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Trabajadores');XLSX.writeFile(wb,`trabajadores_${hotelDetail.name.replace(/[^a-z0-9]+/gi,'_')}_${new Date().toISOString().slice(0,10)}.xlsx`)};
 ...
 {section==='hotels'&&<section className="rounded-2xl bg-white p-5 lg:col-span-2"><h2 className="mb-4 flex gap-2 font-black"><Hotel className="text-slate-900"/>Hospedajes</h2>
  <div className="mb-6"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Sincronizar con un nuevo hospedaje</p><form onSubmit={searchHotel} className="flex gap-2"><input className={field} placeholder="Buscar por nombre o RUC del hospedaje" value={hotelSearch} onChange={e=>setHotelSearch(e.target.value)}/><button type="submit" disabled={hotelSearchBusy} className="flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{hotelSearchBusy?'Buscando...':'Buscar'}</button></form>
   {hotelSearchResults!==null&&<div className="mt-3">{hotelSearchResults.length===0?<p className="text-sm text-slate-400">No se encontró ningún hospedaje con ese nombre o RUC.</p>:hotelSearchResults.map(h=><button key={h.id} type="button" disabled={busy} onClick={()=>void link(h.id)} className="mb-2 flex w-full justify-between rounded-xl border p-3 text-sm font-bold disabled:opacity-40">{h.name}<Plus className="h-4 w-4 text-slate-900"/></button>)}</div>}
  </div>
  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Tus hospedajes</p>
  {data?.hotels.map(h=><div key={h.link_id} className="mb-2 flex items-center justify-between rounded-xl border p-3"><b className="text-sm">{h.name}</b><div className="flex items-center gap-2">{h.status==='accepted'&&<button type="button" onClick={()=>void openHotelDetail(h)} className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200"><Info className="h-3.5 w-3.5"/>Ver detalles</button>}{h.status!=='accepted'&&<span className="text-xs font-bold uppercase text-amber-600">{h.status==='pending'?'Pendiente':h.status==='revoked'?'Desincronizado':h.status==='rejected'?'Rechazado':h.status}</span>}{(h.status==='revoked'||h.status==='rejected')&&<button type="button" disabled={busy} onClick={()=>void link(h.tenant_id)} className="flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-bold text-slate-900 hover:bg-amber-600 disabled:opacity-40">Solicitar de nuevo</button>}</div></div>)}
 </section>}
 ...
 {hotelDetail&&!showValuation&&<div className={iosOverlay}><div className={iosSheet.replace('sm:max-w-md','sm:max-w-3xl')}>
  <div className="mx-auto mt-2 h-1.5 w-10 rounded-full bg-slate-300 sm:hidden"/>
  ...
  <div className="mt-4 px-4"><div className={iosDetailGroup}>
    <div className={iosDetailRow}><span className="flex items-center gap-2 text-xs text-slate-500"><Phone className="h-4 w-4 shrink-0 text-slate-400"/>Teléfono</span><span className="text-lg font-semibold text-slate-900">{hotelDetail.contact_phone||'No registrado'}</span></div>
    <div className={iosDetailRow}><span className="flex items-center gap-2 text-xs text-slate-500"><BedDouble className="h-4 w-4 shrink-0 text-slate-400"/>Habitaciones disponibles</span><span className="text-lg font-semibold text-slate-900">{hotelDetail.rooms_available} / {hotelDetail.rooms_total}</span></div>
    <div className={iosDetailRow}><span className="flex items-center gap-2 text-xs text-slate-500"><Users className="h-4 w-4 shrink-0 text-slate-400"/>Trabajadores ahí</span><span className="text-lg font-semibold text-slate-900">{hotelDetail.workers_here}</span></div>
  </div></div>
  <div className="mt-5 px-4"><div className="mb-1.5 flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Trabajadores en el hospedaje</p>{hotelDetail.workers_here_list.length>0&&<button type="button" onClick={exportHotelWorkers} className="flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700 hover:bg-slate-200"><Download className="h-3.5 w-3.5"/>Exportar</button>}</div>
   {hotelDetail.workers_here_list.length===0?<p className="rounded-2xl border border-slate-200 bg-white px-4 py-6 text-center text-sm text-slate-400">Ningún trabajador tuyo está hospedado ahí ahora.</p>:
   <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[480px] text-left text-xs"><thead><tr className="border-b bg-slate-50 text-slate-500"><th className="px-3 py-2">DNI</th><th className="px-3 py-2">Nombre</th><th className="px-3 py-2">Hab.</th><th className="px-3 py-2">Entrada</th><th className="px-3 py-2">Salida</th></tr></thead><tbody>{hotelDetail.workers_here_list.map((w,i)=><tr key={i} className="border-b last:border-0"><td className="px-3 py-2 font-mono">{w.guest_dni}</td><td className="px-3 py-2 font-semibold">{w.guest_name}</td><td className="px-3 py-2">{w.room_number||'—'}</td><td className="px-3 py-2">{fmtDate(w.check_in_date)}</td><td className="px-3 py-2">{w.check_out_date?fmtDate(w.check_out_date):'Sin definir'}</td></tr>)}</tbody></table></div>}
  </div>
  ...
  <button type="button" disabled={busy} onClick={()=>void unlinkHotel()} className="mx-4 mt-2 flex items-center justify-center gap-2 rounded-full py-2.5 text-sm font-bold text-red-600 disabled:opacity-40">Desincronizar hospedaje</button>
 </div></div>}
